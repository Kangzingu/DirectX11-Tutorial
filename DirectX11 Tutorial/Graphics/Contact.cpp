...
void Contact::CalculateInternals(float deltaTime)
{
	...
	Vector3 closingVelocity = m_objects[0]->rigidbody.GetVelocity();
	if (m_objects[1] != nullptr)
	{
		closingVelocity -= m_objects[1]->rigidbody.GetVelocity();
	}
...
{
	this->m_relativeContactPosition[0] = m_point - m_objects[0]->transform.GetPosition();
	if (m_objects[1] != nullptr)
	{
		this->m_relativeContactPosition[1] = m_point - m_objects[1]->transform.GetPosition();
	}
}
...
{
	Vector3 deltaVelWorld = Vector3::Cross(m_relativeContactPosition[0], m_normal);
	deltaVelWorld = m_objects[0]->rigidbody.GetWorldInertiaTensorInverse() * deltaVelWorld;
	deltaVelWorld = Vector3::Cross(deltaVelWorld, m_relativeContactPosition[0]);
	float deltaVelocity = Vector3::Dot(deltaVelWorld, m_normal);
	deltaVelocity += m_objects[0]->rigidbody.GetInverseMass();

	if (m_objects[1] != nullptr)
	{
		deltaVelWorld = Vector3::Cross(m_relativeContactPosition[1], m_normal);
		deltaVelWorld = m_objects[1]->rigidbody.GetWorldInertiaTensorInverse() * deltaVelWorld;
		deltaVelWorld = Vector3::Cross(deltaVelWorld, m_relativeContactPosition[1]);
		deltaVelocity += Vector3::Dot(deltaVelWorld, m_normal);
		deltaVelocity += m_objects[1]->rigidbody.GetInverseMass();
	}
...
void Contact::ResolveVelocity()
{
	...
	Vector3 impulsiveTorque = Vector3::Cross(m_relativeContactPosition[0], impulse);
	Vector3 angularVelocityChange = m_objects[0]->rigidbody.GetWorldInertiaTensorInverse() * impulsiveTorque;
	Vector3 linearVelocityChange = impulse * m_objects[0]->rigidbody.GetInverseMass();

	m_objects[0]->rigidbody.AddVelocity(linearVelocityChange);
	m_objects[0]->rigidbody.AddAngularVelocity(angularVelocityChange);

	if (m_objects[1] != nullptr)
	{
		Vector3 impulsiveTorque = Vector3::Cross(impulse, m_relativeContactPosition[1]);
		angularVelocityChange = m_objects[1]->rigidbody.GetWorldInertiaTensorInverse() * impulsiveTorque;
		linearVelocityChange = -impulse * m_objects[1]->rigidbody.GetInverseMass();

		m_objects[1]->rigidbody.AddVelocity(linearVelocityChange);
		m_objects[1]->rigidbody.AddAngularVelocity(angularVelocityChange);
	}
}
