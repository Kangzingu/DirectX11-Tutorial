...
void Engine::InitializeScene()
{
	...
	gameObjects.push_back(gameObject);
	gameObjects.push_back(gameObject);
	gameObjects[0].SetScale(100.0f, 0.1f, 100.1f);
	for (int i = 0; i < gameObjects.size(); i++)
		gameObjects[i].SetPosition(i * 2.0f, i*0.3f, 0.0f);

	light.Initialize(device.Get(), deviceContext.Get(), vsConstantBuffer, aiColor3D(0.0f, 0.0f, 0.0f));

	camera.SetPosition(2.0f, 3.0f, -10.0f);
...
void Engine::HandleEvent()
{
	...
	while (!windowManager.mouse.IsEventBufferEmpty())
	{
		...
		{
			this->camera.AdjustRotation((float)me.GetPosY() * 0.001f, (float)me.GetPosX() * 0.001f, 0);
		}
	}
	...
	{
		this->camera.AdjustPosition(this->camera.GetForwardVector() * cameraSpeed * deltaTime);
	}
	if (windowManager.keyboard.KeyIsPressed('A'))
	{
		this->camera.AdjustPosition(this->camera.GetLeftVector() * cameraSpeed * deltaTime);
	}
	if (windowManager.keyboard.KeyIsPressed('S'))
	{
		this->camera.AdjustPosition(this->camera.GetBackwardVector() * cameraSpeed * deltaTime);
	}
	if (windowManager.keyboard.KeyIsPressed('D'))
	{
		this->camera.AdjustPosition(this->camera.GetRightVector() * cameraSpeed * deltaTime);
	}
	if (windowManager.keyboard.KeyIsPressed(VK_SPACE))
	{
		this->camera.AdjustPosition(0.0f, deltaTime, 0.0f);
	}
...
void Engine::UpdateScene()
{
	...
	psConstantBuffer.data.dynamicLightPosition = light.GetPositionFloat3();
	...
	}
	{
		...
		CreateInputLayoutFromEffect<DirectX::VertexPositionColor>(device.Get(), m_effect.get(),
			m_inputLayout.ReleaseAndGetAddressOf());

		m_batch = make_unique<PrimitiveBatch<VertexPositionColor>>(deviceContext.Get());
		...
		//	m_batch->DrawLine(v1, v2);
		//}
		DirectX::XMVECTORF32 lineColor;
		XMFLOAT3 waterSurface = gameObjects[0].GetPositionFloat3();
		XMVECTOR waterSurfaceVector;
		XMVECTOR halfHeight;
		for (int i = 1; i < gameObjects.size(); i++)
		{
			waterSurface.x = gameObjects[i].GetPositionFloat3().x;
			waterSurface.z = gameObjects[i].GetPositionFloat3().z;
			waterSurfaceVector = XMLoadFloat3(&waterSurface);
			VertexPositionColor v1(waterSurfaceVector, DirectX::Colors::White);
			float lineLength = XMVectorGetX(XMVector3Length(waterSurfaceVector -gameObjects[i].GetPositionVector()));
			lineColor = { 1.0f / lineLength, 1.0f/ lineLength, 1.0f, 1.0f };
			halfHeight = XMVectorSet(0.0f, gameObjects[i].GetScaleFloat3().y, 0.0f, 0.0f);
			VertexPositionColor v2(gameObjects[i].GetPositionVector(), lineColor);
			m_batch->DrawLine(v1, v2);
		}
		m_batch->End();
	}
